  ...
  return (
    <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-5 lg:px-8">
        <a href="/" className="font-display text-3xl tracking-tight text-ink">Siroi</a>
        ...
        <a href="#download" className="hidden rounded-full bg-accent px-6 py-2.5 text-sm text-white transition-transform hover:scale-[1.03] lg:inline-flex">
          {NAV_CTA}
        </a>
        <button
          type="button"
      ...
      <dialog ref={dialog} aria-label="Menu" className="m-0 h-[100dvh] w-full max-w-none bg-white p-6 text-ink backdrop:bg-accent/20" onClick={(e) => e.target === dialog.current && dialog.current?.close()}>
        ...
        <nav aria-label="Menu" className="mt-6 flex flex-col">
          ...
          <a href="#download" className="mt-8 inline-flex min-h-14 items-center justify-center rounded-full bg-accent px-8 text-base text-white" onClick={() => dialog.current?.close()}>{NAV_CTA}</a>
        </nav>
      </dialog>
    </header>
  );
