import { PageAssembler, toPages } from '@hillpath/core';
...
/** Shows text as looping QR frames. Any frame order works, so a scanner can join at any point. Has a pause control. */
export function QRShow({ text, label }: { text: string; label: string }) {
  const pages = useMemo(() => toPages(text, PAGE), [text]);
  ...
  useEffect(() => {
    ...
    const t = window.setInterval(() => setI((x) => (x + 1) % pages.length), FRAME_MS);
    return () => window.clearInterval(t);
  }, [paused, pages.length]);

  useEffect(() => {
    if (canvas.current) void QRCode.toCanvas(canvas.current, pages[i % pages.length]!, { errorCorrectionLevel: 'L', margin: 2, width: 320 });
  }, [i, pages]);

  return (
    <figure className="flex flex-col items-center gap-3" data-testid="qr-show">
      ...
      <figcaption className="text-base" aria-live="off">
        Frame {(i % pages.length) + 1} of {pages.length}. Hold the other phone steady in front of this screen.
      </figcaption>
      {pages.length > 1 && (
      ...
      <textarea readOnly className="field sr-only" aria-label="Code text for pasting" value={pages.join('\n')} data-testid="qr-text" />
    </figure>
  );
}

/** Camera scanner with a paste fallback. Collects pages until the text is complete. */
