...
function RoleChooser() {
  ...
  return (
    <main className="patient mx-auto flex min-h-[100dvh] max-w-2xl flex-col gap-4 px-4 py-8">
      ...
      <BigButton onClick={() => void core.setRole('health_worker').then(() => go('caregiver', 'home'))} className="w-full">A community health worker</BigButton>
...
function CaregiverApp({ route }: { route: string[] }) {
  const core = useCore();
  const tab = TABS.some(([k]) => k === route[1]) ? route[1]! : 'home';
